...
static bool GBASIOLockstepCoordinatorStart(struct GBASIOLockstepCoordinator*, uint32_t id);

static void GBASIOLockstepCoordinatorWaitOnPlayers(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void GBASIOLockstepCoordinatorAckPlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void GBASIOLockstepCoordinatorWakePlayers(struct GBASIOLockstepCoordinator* coordinator);
static void GBASIOLockstepCoordinatorWakePlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void GBASIOLockstepCoordinatorSleepPlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);

static int32_t GBASIOLockstepTime(struct GBASIOLockstepPlayer*);

static void _advanceCycle(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void _removePlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void _reconfigPlayers(struct GBASIOLockstepCoordinator* coordinator);
static int32_t _untilNextSync(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
static void _enqueueEvent(struct GBASIOLockstepCoordinator* coordinator, const struct GBASIOLockstepEvent* event, uint32_t target);
static void _setReady(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* activePlayer, int playerId, enum GBASIOMode mode);
static void _hardSync(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player);
...
static bool GBASIOLockstepDriverStart(struct GBASIODriver* driver) {
	struct GBASIOLockstepDriver* lockstep = (struct GBASIOLockstepDriver*) driver;
	return GBASIOLockstepCoordinatorStart(lockstep->coordinator, lockstep->lockstepId);
}
...
void _setReady(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* activePlayer, int playerId, enum GBASIOMode mode) {
	...
	bool ready = mode == activePlayer->mode;
	int i;
...
bool GBASIOLockstepCoordinatorStart(struct GBASIOLockstepCoordinator* coordinator, uint32_t id) {
	bool ret = false;
	MutexLock(&coordinator->mutex);
	if (coordinator->transferActive) {
	...
	struct GBASIOLockstepPlayer* player = TableLookup(&coordinator->players, id);
	if (player->playerId != 0) {
		// TODO: handle this for normal mode edge cases
...
void _lockstepEvent(struct mTiming* timing, void* context, uint32_t cyclesLate) {
	...
	if (player->playerId != 0) {
		if (nextEvent <= LOCKSTEP_INCREMENT && !player->queue) {
			GBASIOLockstepCoordinatorSleepPlayer(coordinator, player);
...
void GBASIOLockstepCoordinatorWakePlayers(struct GBASIOLockstepCoordinator* coordinator) {
	...
	for (i = 1; i < coordinator->nAttached; ++i) {
		...
		struct GBASIOLockstepPlayer* player = TableLookup(&coordinator->players, coordinator->attachedPlayers[i]);
		GBASIOLockstepCoordinatorWakePlayer(coordinator, player);
	}
}

void GBASIOLockstepCoordinatorWakePlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player) {
	if (!player->asleep) {
		return;
	}
	player->asleep = false;
...
void GBASIOLockstepCoordinatorAckPlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player) {
	...
	if (!coordinator->waiting) {
		...
		GBASIOLockstepCoordinatorWakePlayer(coordinator, runner);
	}
	GBASIOLockstepCoordinatorSleepPlayer(coordinator, player);
}

void GBASIOLockstepCoordinatorSleepPlayer(struct GBASIOLockstepCoordinator* coordinator, struct GBASIOLockstepPlayer* player) {
	if (!player->asleep) {
